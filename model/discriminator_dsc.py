...
class DSCDiscriminator(nn.Module):

	def __init__(self, num_classes, ndf = 64):
		super(DSCDiscriminator, self).__init__()
		...
		self.depthwiseCL = nn.Conv2d(ndf*8, ndf*8, kernel_size=4, padding=1, groups=ndf*8, bias=False, stride=2)
		self.pointwiseCL= nn.Conv2d(ndf*8, 1, kernel_size=1, bias=False)
	...
	def forward(self, x):
		...
		x = self.depthwiseCL(x)
		x = self.pointwiseCL(x)
